...
var utils = {
	...
	deadSpec(spec) {
		...
		if (spec.ministry == 'OIA') {
			if (!(content.works[utils.getCurrentWork(spec).id].type.has('relax'))) {
			utils.callPopup({
				text:(strings.UI.messages.deadSpec.replace('%spec%',spec.stats.name))+'\n('+game.player.resources.money.value+')',
				buttons:[{
	...
	calcPayout(spec) {
		...
		if (x/spec.attributes.payout > 1.02) x = parseInt(spec.attributes.payout*1.02);
	...
	destroyWork(work, res) {
		...
		if (playerSpecs) {
			...
			else if (res == 5) utils.addNotify('specs', work.workers, 0, strings.UI.messages.workLowRes.replace('%work%', pat.name));
		}
			...
			for (let j=0; j<s.length; j++) {
				if (s[j] == work.internalId) {
					...
					if ((spec.internalId == 'GB') && (spec.location != world.homecity) && (spec.tasks.length == 0))
						utils.startTask(content.works.w_movement, [spec.id], spec.ministry, undefined, world.homecity);
					break;
				}
			}
		}
	...
	startTask(task, spec, ministry, targetSpec, location, insertBefore) {
		...
		world.tasks[i] = new work(task.id, spec, location, ministry, targetSpec, i, timeout);
		...
		if (popUp.buttons.length>0) {
			popUp.buttons.push({
				...
				callback:function() {
					...
					game.UI.taskHandling.targetSpec = targetSpec;
					if (specList == undefined) specList = [];
					game.UI.taskHandling.workers = specList;
					utils.closePopup();
					utils.callTaskWindow();
				}
			});
	...
	normalCityTick(city) {
		/*
			Oh, you expected something useful here? 
	...
	newDayTick() {
		...
		let tpartSum = 0;
		let ministryRandom = parseInt(Math.random()*consts.actualMinistries.length);
