...
function compileMap() {
	...
	while (c != undefined) {
		...
		if (!m.multilevel) map.push(m.id);
			...
			es.m.appendChild(InfernoAddElem('div', {className:'l2c'}, els));
		}
		c = m.next;
	}
}
...
function postProcessor() {
	...
	var props = {};
	var scount = 1;
	var metaTags = ['zoomImages', 'combineImages', 'showLabels', 'continueGallery', 'fullGif', 'floatRight', 'applyToNext', 'nativeHTML', 'skipCurrent', 'addLinkUrl'];
	...
	for (var i=0; i<p.length; i++) {	
		...
		if (props.addLinkUrl) {
			for (var j=0; j<as.length; j++) {
				if (as[j].hostname != 'web.archive.org') as[j].innerHTML += (' // '+decodeURI(as[j].href));
			}
		}
